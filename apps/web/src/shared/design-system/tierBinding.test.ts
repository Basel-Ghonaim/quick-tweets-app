 ...
 * reaches for the semantic and intent tiers, never for a raw primitive scale.
 *
 * The token checker proves a reference *resolves*, which a primitive reference
 * does — so it cannot see the tier a reference resolved to, and three component
 * migrations enforced this rule by reading the diff.
 *
 ...
 * binds them directly — that is what a curated scale is for. Palette and
 * typography are raw, and must reach a component through an intent.
 *
 ...
 * legitimately composes with the curated scales.
 */
