...
Where a platform component must *render* something the application owns, the element is taken rather than the dependency. Navigation is the case: the router is mounted by the composition root and a router's link throws outside it, so a presentation component that imported one could not render on its own. **A seam is the answer when the platform must call the application; taking the element is the answer when it only has to render one** — and when a second component needs to navigate, that repetition earns the seam.

**Navigation has now converted.** `Link` took the element from each caller while it was the only thing that navigated; a set of tabs and an action drawn as a destination made three, so the element is registered **once** with the Design System by the composition root, and every destination in the layer renders with it. The registration is restricted by the props the element must accept rather than by a list of tags, which is why the layer still holds no router and needs none — and, unregistered, it falls back to a plain anchor, so the layer renders outside the application at all. **The conversion is what the rule above prescribed, not an exception to it.**
...
The design system's foundations (tokens and themes) are loaded once at the entry as a side effect, so every feature renders against the same visual base. **Which** theme, and which language the interface is in, are settled before any of that, and the direction the document reads in follows the language rather than being chosen. The theme is resolved by the Design System and *selected* by the application ([ADR 0010](../architecture/decisions/0010-design-system-platform-reestablishment.md) Decision 4); the language is the reader's, resolved as the [localisation contract](localisation.md) states. Both are stamped on the document from the markup, ahead of the first paint, because a module cannot run early enough to avoid a flash.
...
| `localisation/` | mechanism | which catalogue the active language reads from, and how a language writes numbers; it holds no catalogue of its own | [Frontend Localisation](localisation.md) |
| `one-time-code/` | mechanism | how a typed code is normalised before anything reads it | this document, until it has a stable core |
| `preferences/` | mechanism | the reader's preferences — which theme resolves the design language, and which language the interface is in, with the document's direction following it | [Frontend Localisation](localisation.md) for the language; this document for the theme, until it has a stable core |
...
| `copy/` | product content | user-facing text addressed by key, namespaced by surface, one catalogue per language in English's shape, read through its accessors and never imported outward by a platform capability | [Frontend Localisation](localisation.md) for the catalogues' shape and how they are read; this document for the rest, until it has a stable core |
