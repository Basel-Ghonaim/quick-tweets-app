...
> This document owns the frontend schema-driven form engine. The presentation controls are owned by the [design system](design-system.md), submission-error normalization by [error handling](error-handling.md), the application layout by the frontend architecture document (forthcoming), per-feature usage by the feature documents, and the underlying principles by [Engineering Principles](../development/engineering-principles.md) — linked here, never duplicated. The recorded schema-form ↔ design-system deviation lives in [Finding 0001](../architecture/findings/0001-schema-form-design-system-cycle.md).
