...
- **Validation.** Each field carries an ordered list of validators; the engine runs them as the field changes (first failure wins) and sweeps every field once more before submitting. Every validator factory **takes the message it reports** — the engine holds no default words, so a message is always the schema's, drawn from the product's content.
