...
M6 deliberately exercises only the *reference-begins* half (ingest + attach on the same being-created principal), so it needs **neither** cross-principal attach-authorization/quota (M7) **nor** the *reference-ends* coordination — which is exactly why it is the minimal proof and comes first. Generalization is sequenced *after* #256 so the first milestone stays a clean, uncontended vertical, even though **M8** depends only on the registry and **M7** only on the registry plus the ingest boundary — both already built by Phase 2. Lifecycle is independent of the consumers' *code* but **not** of their *coordination*: reclamation (M11) must not be enabled until every reference-creating consumer signals referenced-ness through the coordination mechanism (**M8a**), or an established reference could be misread as garbage. **Sequencing within Phase 5 — data-safety-first:** M10 (the substrate) → **Comment Media** (a third reference *producer*: single reference per comment, the avatar shape) → **verification** of its begin/end coordination → **then** M11. The reason is specific: M11's producer-agnostic design (it reads only the ledger, never feature schemas) protects it from a producer's *end* mistakes (which merely leak, conservatively) but **not** from a producer's *missing-begin* (a live object misread as garbage → irreversible deletion). The only defence against that is verifying every producer's coordination **before** physical reclamation is switched on. Comment Media is a committed near-term producer, so it is brought in — and verified with the existing manual harness — before M11, so the destructive switch is thrown **once**, against a fully-exercised ledger. M11 additionally ships **report-only first** (identify + log, delete nothing) as insurance on an irreversible operation. M12 closes the effort once the mechanisms are real in code.
...
| **4 — Generalization & second consumer** | Cross-principal authority + `TweetMedia` schema + reference coordination + tweet media. | M7, M8, M8a, M9 (M9b deferred) |
| **5 — Lifecycle & reclamation** | The first background-execution substrate, a third media consumer, then Media-owned reclamation. | M10, Comment Media, M11 |
| **6 — Reconciliation** | Create `media.md`; reduce ADR 0005 to boundary + rationale. | M12 |

**Decision gates** (deferred decisions pulled just-in-time — see §5): the pre-auth-ingest sub-ADR and the **begin-half** of the reference-coordination mechanism **before M6**; the validation technique **before M3**; the **full** reference-coordination contract (begin *and* end) **before M9 and M11** — settled and built by **M8a**, which the gate resolved into its own Work Item rather than a sub-ADR (see §5); the background-execution substrate **within M10, before M11**. The quota-policy value, the media access posture, and the object-store extension are **not** on the critical path to any local-disk milestone (including #256).
...
- **M8a · Reference coordination — how a feature signals a reference begins and ends.** *(Decision 8; deps: M2)* Resolve the deferred coordination gate and build the mechanism: the feature owns the **trigger**, Media owns the **write**, and the write is **transactionally bound** to the feature's own transaction so a signal cannot be lost half-way (the shape M6's adoption already proved). Media then determines referenced-ness from **its own state**, never from feature schemas. Deliberately **model-neutral**: whether a `MediaObject` is a *reusable asset* or a *disposable one-time upload* is a **product** decision left open, so the mechanism must not force either. *Added after M8; the gate's resolution — a Work Item rather than a sub-ADR — is recorded in §5.* *Why:* M8's `Restrict` makes a tweet carrying media undeletable until this exists, so it is a hard prerequisite for M9 and for reclamation (M11).
- **M9 · Tweet media — ordered references, attach-authorization, coordination (backend).** *(Decisions 9, 5, 3, 8; deps: M4, M5, M7, M8, M8a)* Wire tweet create/edit to accept ordered *references* only, attach through Media under the authenticated author, persist ordered `TweetMedia`, coordinate every reference begin/end, and surface the collection via the read endpoint. Also closes the coordination gap left by M6 — the avatar reference must participate too (including a backfill), since reclamation is gated on **every** consumer signalling. First path to exercise *reference-ends* coordination (replace/remove/delete). *Why:* the richer consumer, and the milestone that completes M11's prerequisite.
- **M9b · Tweet-compose media (dropzone) — the client half.** *(Decisions 9, 3; deps: M9; reuses M6's client pattern)* The compose/feed surface that lets a person actually attach media: the tweets frontend module, the upload-then-submit-reference flow under an authenticated principal, and the existing dropzone `FileInput` wired into a real form. **Deferred until the tweets UI/UX is designed** — page structure, routing, and state management are undecided, and the media boundary must not be coupled to unfinished UI decisions. *Split out of M9 rather than dropped: the media effort is not complete until a user can attach media.*

**Phase 5 — Lifecycle & reclamation**

- **M10 · Scheduled/background-execution substrate (platform capability).** *(Decision 8; deps: —)* Introduce the codebase's first background-execution mechanism generically (single-run safety under concurrency/restarts, failure isolation, observability); it is reusable, and its **first job is refresh-token cleanup** — a safe, non-destructive proving workload — with Media reclamation registering later (M11). Single-run is a **session-scoped Postgres advisory lock on a dedicated connection** (not transaction-scoped, so the handler runs under no enclosing transaction). *Why:* ADR 0005 names this a new platform concern for its own Work Item; reclamation cannot run without it, and proving the substrate on a recoverable job first de-risks the destructive one.
- **Comment Media · a third media consumer (single reference per comment).** *(Decisions 9, 3, 5, 8; deps: M4, M5, M7, M8a; reuses M6's client pattern)* The avatar-shaped producer — a bare `mediaId` reference on `Comment`, referrer tag `comment:{id}`, no join table — so M8a already generalized for it with no new mechanism or schema decision. Introduces one genuinely new coordination case: a **transitive reference-end**, since deleting a Tweet cascades to its Comments and must end their media references. Sequenced (and verified) **before M11** so every reference producer is faithful before physical reclamation is enabled. *Why:* the data-safety rationale in §3 — M11 cannot self-defend against a producer's missing-begin; only pre-enablement verification can.
- **M11 · Media reclamation of unreferenced objects.** *(Decisions 8, 9; deps: M10, M2, M8, + every reference producer verified)* Media's exclusive physical deletion, running on the substrate: reclaim now-unreferenced owned objects — the single reclamation class after the pre-auth grant's "abandoned" class was retired (ADR 0008) — determining referenced-ness from the registry (never feature schemas), under the conservative invariants (an object is never reclaimed before it can be referenced; single recovery unit; quarantine-on-divergence). Reclamation is **enabled only after** every reference-creating consumer (M6, M9, **Comment Media**) participates in the coordination mechanism and is verified — otherwise an established reference could be misread as never-referenced; retiring the untracked `Tweet.image` path (M8) is a precondition for enabling physical deletion. Ships **report-only first** (identify + log, delete nothing) before physical deletion is switched on. A tombstoned row whose bytes still linger after a crashed/failed post-commit delete is **recovered** on a later pass (an idempotent byte re-delete, the tombstone permanently retained) — *retryable cleanup*, not a quarantine divergence ([#356](https://github.com/Basel-Ghonaim/quick-tweets-app/issues/356)); that recovery and a controlled destructive-path verification ([#375](https://github.com/Basel-Ghonaim/quick-tweets-app/issues/375)) are the hard prerequisites for switching physical deletion on. *Why:* the *reference-ends*/cleanup half, separated from the first-consumer attach path.
...
| 8 — Lifecycle/deletion/reclamation; conservative invariants | M8a, M10, M11, M9 |
...
- **Reference-coordination mechanism** — *was due before M9 and M11 (the begin-half touches M6).* How a feature signals a reference begins/ends so Media reasons from its own state (explicit call, transactional bookkeeping, reference count, or reconciliation). Cross-cutting across the consumers and reclamation. **The M6 begin-half is fixed by [ADR 0007](../architecture/decisions/0007-pre-auth-ingest-upload-grant-model.md)** — adoption constitutes the reference-begins signal on the register path. **The general begin/end contract resolved to a Work Item (M8a), not a sub-ADR** — applying [ADR 0002](../architecture/decisions/0002-refined-adr-threshold.md), it has a natural owner (ADR 0005's Consequences already route this mechanism to the future `docs/backend/media.md`) and invents no boundary, since Decision 8 declared all four mechanisms admissible. *This is the one gate listed here that did not become a sub-ADR — the record type was decided by the analysis, not assumed by the plan.* It would rise to an ADR only if the design required a new `MediaObject` lifecycle state; it does not. **Resolved:** an explicit, transactionally-bound signal over a **reference ledger** — chosen against a marker, a count, and reconciliation-as-primary, with the comparison recorded in the M8a Work Item until `media.md` (M12) takes ownership.
