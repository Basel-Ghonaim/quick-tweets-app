...
2. **Frontend cutover precedes grant-path removal** — no runtime dual-protocol window.
...
- **WI-6 · Remove grant-referencing code.** *(D3, D5, D8, D10; deps: WI-4)* Delete grant mint/verify, adoption (`IMediaAdoption`, `adoptById`), grant provenance handling (`MediaProvenance`/repo/types), the Auth→Media `adoption`/`references` ports (Auth becomes **Media-free**; avatar resolution now lives in User, WI-2), **and M11's abandoned selection** (`findAbandoned` + the `abandoned` reason) since it reads `grantExpiresAt`. Must land **before** WI-7. *Why:* invariant 3 — nothing may reference a column about to be dropped.
...
- **WI-8 · Reconcile M11 to the single unreferenced-owned class.** *(D8, D13, D14; deps: WI-6 ∧ WI-7)* Simplify `ReclaimReason` and audit terminology to the single class; update the [Media execution plan](media-implementation.md) (M11 description), the [verification catalogue](../development/verification/verification-scenarios.md) (retire the abandoned scenarios), and api-contract/data-model wording. Confirm **report-only, single-class** operation. **Hands off** to the M11 destructive-verification effort (#354/#356 + the controlled oracle), which builds against this final lifecycle and is **out of this plan's scope**. *Why:* leaves the Media lifecycle in its final, verifiable form.
