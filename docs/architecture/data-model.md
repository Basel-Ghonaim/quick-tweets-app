...
- An **`OnboardingJourney`** belongs to a user, **at most one, forever** — `user_id` is unique, and only registration creates one. It records how far a reader got through the post-registration steps so the client can decide which onboarding screens to render; nothing an account may *do* is gated on it ([ADR 0008](decisions/0008-auth-first-onboarding-grant-retirement.md) Decision 2, as revised). It stores no phase: `profile` / `verify` / `code` / `none` is derived at read time from three set-once timestamps, each written only over `NULL`, so the record is monotonic by construction and two concurrent writers produce one write and one no-op rather than a lost update. What it does store is **how each optional step ended**, and the two are captured differently on purpose. The profile step's outcome is **asserted by the client**, because saving a sparse profile and skipping it leave identical data behind and the reader's choice is the only witness. The verification step's is **read at the close from Channel Verification**, which is the sole authority on whether a channel is proven — so the row holds an observation of someone else's fact rather than a second copy free to disagree. Each is written with the mark it belongs to, under a constraint that the mark cannot exist without it. Nothing deletes the row and there is **no sweep** — unlike the two challenge tables, whose rows are unbounded per user, this is one row per account, and the surviving row is precisely what makes a finished journey permanently finished rather than merely missing.
...
- A **`PasswordResetChallenge`** carries the **address it was minted for**, frozen. A completed reset proves that endpoint rather than the account's address at the time of writing, since one changed mid-flow was never demonstrated ([ADR 0017](decisions/0017-recovery-session-and-the-proof-a-reset-produces.md) Decision 6).
