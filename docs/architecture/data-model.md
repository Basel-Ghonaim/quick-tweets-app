...
- deleting a **user** removes their tweets, comments, likes, follows (in both directions), refresh tokens, username aliases, and password reset challenges — a user's entire footprint leaves with them (with **one exception**: a user that *owns media objects* is blocked from deletion by the uploader `Restrict` foreign key — see the media note below);
