...
1. **`state-and-data.md` deferred (row 11, order step 4).** A maturity review during execution found the RTK Query cache/data layer unrealized in code (no injected endpoints, tags, or hooks) — a platform document authored then would have described a plan, not an implementation. Its authoring is **deferred until the data layer exists**; the re-evaluation trigger is the first real RTK Query endpoint ([#185](https://github.com/Basel-Ghonaim/quick-tweets-app/issues/185) / [#186](https://github.com/Basel-Ghonaim/quick-tweets-app/issues/186)). Until then `src/docs/rtk-query-strategy.md` deliberately remains in place as the interim source, and row 11's merge-and-delete executes when the deferred document is authored. The migration completes **with this one explicit deferral**.
