...
- [Documentation Strategy](../documentation-strategy.md) §3 carries the operative Stable-Core rule; it is now binding governance for when a platform document is created. §4's section-level rule ("sections appear only when the code exists") cross-references it as the whole-document analog.
...
- This ADR is immutable once accepted; its status moves from `Proposed` to `Accepted` on merge. A future change to the rule is a new, superseding ADR.
