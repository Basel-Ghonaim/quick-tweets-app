...
1. **The governing documents still describe the pre-split world.** The [Documentation Strategy](../documentation-strategy.md)'s directory structure (§4) and the [Migration Plan](../documentation-migration-plan.md) still name a single `workflow.md`; [CLAUDE.md](../../../CLAUDE.md)'s Authoritative Documents still point to the legacy `src/docs/Principles/*` files.
...
2. **The Constitutional Document set is defined** and owned by the Documentation Strategy. The constitution comprises the small set of governing documents that define how the project's documentation, engineering, and execution work:
   ...
   The [Migration Plan](../documentation-migration-plan.md) is a binding *execution plan*, not a governing constitution, and is excluded.
