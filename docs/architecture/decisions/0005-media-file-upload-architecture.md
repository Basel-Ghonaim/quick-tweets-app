...
8. **Lifecycle and centralized deletion/cleanup ownership.** A `MediaObject` moves `pending → ready → deleted`, with `failed` for validation/store failures; a reference is `referenced` or `unreferenced`. Ownership is single-owner per responsibility:
   ...
   - **Features own:** domain reference persistence (`User.profileImage`, `TweetMedia`) and the decision to **attach / detach / replace** a reference. Features never call storage deletion.

   Features report attach/detach to a **media-owned reference registry**; deletion authority is centralized in Media. A **Grace TTL** (never delete an object younger than the TTL or still `pending`) is **paired with a periodic reconciliation sweeper** that (a) removes `ready`-but-unreferenced objects past grace, (b) removes `pending`/`failed` objects past a longer TTL, and (c) reconciles registry ↔ storage — deleting leaked objects and flagging missing or checksum-mismatched bytes. Media **never scans feature tables** (rejected as a boundary leak).

9. **Data model — fit-to-cardinality persistence.** Multiple tweet images are modeled by a dedicated **`TweetMedia`** entity (one-to-many with `Tweet`, owned by the tweets domain, cascade-deleting its references when a tweet is deleted). The **avatar remains a single reference** on `User`. Both reference `MediaObject`s minted by the one Media subsystem — the mechanism is unified at the capability layer, not the schema.
