...
The fill family carries a resting value and a hover value per role (`--control-fill-<role>` and `--control-fill-<role>-hover`). A filled control's own background **is** its boundary against the surface behind it, so both values are subject to the 3:1 non-text contrast requirement (WCAG 1.4.11), which [ADR 0010](../decisions/0010-design-system-platform-reestablishment.md) Decision 5 places under the system's ownership.
...
The foundation check does not see this: its pairs assert `--control-fill-<role>` against `--surface-page` at 3:1 and never assert `--control-fill-<role>-hover` against any surface. It therefore certifies the resting state and is silent on the state a pointer user is in at the moment they act.

Text contrast is unaffected — `--control-fill-<role>-text` against the hover fill is asserted at 4.5:1 and holds (6.65:1 for `primary` in dark). The gap is the boundary, not the label.
...
Reached in the product by `Button`'s `contained` variant, which binds `--btn-bg-hover` to `--control-fill-<role>-hover` and renders it as both `background-color` and `border-color`. The Storybook accessibility run cannot observe it, because `:hover` is not a rendered state.
...
Resolution has two halves that must land together: correcting the dark theme's hover values so the boundary clears 3:1, and extending the contrast check's pairs to cover `--control-fill-<role>-hover` against every surface. Extending the check first would fail the suite on values that are not yet corrected; correcting the values first would leave the guarantee mechanically unenforced.
