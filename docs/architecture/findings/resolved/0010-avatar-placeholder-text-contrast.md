...
Choosing among those under migration momentum is what the effort's stop rule exists to prevent, and it is the same call [CA-1](../../../plans/historical/component-architecture-unification.md) made when the Pilot Field's gate failed on a legacy colour.
