...
It also hides the real problem. What is actually wrong with these rules is that **no tier owns them** ([Finding 0026](../open/0026-no-tier-owns-the-credential-rules.md)) — a structural weakness the values' current agreement conceals. A sentence reporting a drift that has been fixed occupies the place where that would be said.
...
What the sentence becomes — whether it simply drops, or is replaced by a pointer to [Finding 0026](../open/0026-no-tier-owns-the-credential-rules.md). A finding records the deviation; it never schedules the fix.

## Addendum — 2026-09-21, resolved ([#786](https://github.com/Basel-Ghonaim/quick-tweets-app/issues/786))

**The sentence is gone, and what replaced it points at the real problem.** The authentication document no longer reports a drift in the credential rules. Its forms entry now says that the rules are read from the cross-tier definitions and that which tier owns them is open, and it links [Finding 0026](../open/0026-no-tier-owns-the-credential-rules.md), which is what this Finding said the sentence was hiding. The document moved in the same change, to `docs/features/authentication.md`.
