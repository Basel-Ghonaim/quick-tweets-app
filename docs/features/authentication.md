...
1. **Startup restore.** Before the routed app renders, the feature calls the refresh endpoint (the `HttpOnly` cookie travels automatically). Success → the server returns the **full session** (the user and a fresh access token), which hydrates the store — the user is signed in without credentials. Failure → the refresh session has expired, so a clean logged-out state is set and the login screen is shown. The app shell blocks rendering behind this check, so a returning user never flashes a logged-out view. Restore reads nothing from the client; the server is the source of truth for *who* is returning.
